...
<if !$thread and $threads and $pages><body class="mainpage"><else><body class="threadpage"></if>

}.include("include/header.html").q{

<if !$thread and $threads and $pages>
	...
	<div class="innerbox">
		<if $threadlist>
			<div id="threadlist">
				<loop $threadlist>
					...
					<if $list<=IMAGES_PER_PAGE>
						<a href="<var get_reply_link($num,0)>" rel="nofollow"><var $list>: </a>
						<a href="#<var $list>"><var $subject></a>
					<else>
						<a href="<var get_reply_link($num,0)>" rel="nofollow"><var $list>: <var $subject></a>
					</if>
					</span>
				</loop>
			</div>
		...
		<div id="threadlinks">
			...
			<a href="<var expand_filename(BACKLOG_FILE)>">All threads</a>
		</div>
	</div>
	</div>
<elsif $threads and $thread and !$pages>
	<div id="navigation">
		<strong>Navigation:</strong>
		<a href="<var $self>?/read/<var $thread>/">Entire thread</a>
		<a href="<var $self>?/read/<var $thread>/-100" rel="nofollow">First 100 posts</a>		
		<loop [map {+{'start'=\>$_*100+1}} (1..(@{$$threads[0]{posts}})/100)]>
			<a href="<var $self>?/read/<var $thread>/<var $start>-<var $start+99<@{$$threads[0]{posts}}?$start+99:@{$$threads[0]{posts}}>" rel="nofollow"><var $start>-</a>
		</loop>
		<a href="<var $self>?/read/<var $thread>/l50" rel="nofollow">Last 50 posts</a>		
	</div>
</if>
};
...
<loop $posts>
	<if !$parent>
		<h2><a href="<var get_reply_link($num,0)>"><var $subject||substr(strip_html($comment),0,MAX_FIELD_LENGTH)||get_filename($image)||sprintf(S_THREADTITLE,$num)>
		<small>(<var @$__a+$omit>)</small></a></h2>
		<div class="threadnavigation">
			<a href="#menu" title="Jump to thread list">■</a>
			<a href="#9" title="Jump to previous thread">▲</a>
			<a href="#1" title="Jump to next thread">▼</a>
		</div>
	</if>
	<if !$parent>
		<div class="replies">
		<if !$omit><div class="allreplies"><else><div class="finalreplies"></if>
	</if>
	<div class="reply">
		<h3>
			<span class="replynum"><a title="Quote post number in reply" href="javascript:insert('&gt;&gt;<var $num>',<var $parent||$num>)"><var $num></a></span>
			Name: <span class="postername"><var $name></span><span class="postertrip"><var $trip></span> : <var $date>
			<if $image><span class="filename">(Image: <em><var make_size($size)>, <var $width>x<var $height><if $origname>, <span title="<var clean_string($origname)>"><var show_filename($origname)></span></if></em>)</span></if>
			<span class="deletebutton">[<a href="javascript:delete_post(<var $num>)">Del</a>]</span>
		</h3>
		<if $image>
			<if $thumbnail>
	...
	<if $abbrev><div class="replyabbrev"><var sprintf(S_ABBRTEXT,get_reply_link($num,$parent))></div></if>
	<if !$parent and $omit>
		</div>
		...
		<form id="threadform<var $$posts[0]{num}>" action="<var $self>" method="post" enctype="multipart/form-data">
			<input type="hidden" name="task" value="post" />
			<input type="hidden" name="parent" value="<var $$posts[0]{num}>" />
			...
			<table><tbody>
			<tr>
				<td><if !FORCED_ANON><const S_NAME></if></td>
				<td><if !FORCED_ANON><input type="text" name="field1" size="19" maxlength="100" /></if>
					<if ALLOW_LINK>
						<const S_EMAIL>
						<input type="text" name="field2" size="19" maxlength="100" />
					</if>
					<input type="submit" value="Reply" />
					<if !ALLOW_LINK>
						<label><input type="checkbox" name="sage" value="on" /> <const S_NOBUMP></label>
					</if>
				</td>
			</tr>
			<tr><td></td><td><textarea name="field4" cols="64" rows="5"></textarea></td></tr>

			<if $image_inp>
				<tr><td><const S_UPLOADFILE></td><td><input type="file" name="file" size="49" />
				<if $textonly_inp>[<label><input type="checkbox" name="nofile" value="on" /><const S_NOFILE> ]</label></if>
				</td></tr>
			</if>

			<if ENABLE_CAPTCHA>
				<tr><td><const S_CAPTCHA></td><td><input type="text" name="captcha" size="10" autocomplete="off" />
				<img alt="" src="<var expand_filename(CAPTCHA_SCRIPT)>?key=<var get_captcha_key($thread)>&amp;dummy=<var $dummy>" />
				</td></tr>
			</if>
			</tbody></table>
		</form>
		<script type="text/javascript">set_inputs("threadform<var $$posts[0]{num}>")</script>
	</div>
</loop>
</div>

<if !$thread and $postform>
<a name="newthread"></a>
...
<div class="innerbox">
	...
	<form id="threadform" action="<var $self>" method="post" enctype="multipart/form-data">
		...
		<table><tbody>
		<tr>
			<td><const S_SUBJECT></td>
			<td><input type="text" name="field3" size="46" /> <input type="submit" value="Create new thread" />
			<if FORCED_ANON and !ALLOW_LINK><label><input type="checkbox" name="sage" value="on" /> <const S_NOBUMP></label></if></td>
		</tr>
		<if !FORCED_ANON or ALLOW_LINK><tr>
			<td><if !FORCED_ANON><const S_NAME></if></td>
			<td><if !FORCED_ANON><input type="text" name="field1" size="19" maxlength="100" /></if>
				<if ALLOW_LINK>
					<const S_EMAIL>
...
<loop $posts>
	...
	<td><var dec_to_dot($ip,$ipv6)>
		...
		[<a href="<var $self>?admin=<var $admin>&amp;task=ban&amp;num=<var $num>" onclick="return do_ban(this)"><const S_MPBAN></a>]
		[<a href="<var $self>?admin=<var $admin>&amp;task=edit&amp;num=<var $num>"><const S_MPEDIT></a>]
	</td>
